...
def discover_candidates(context=None,symptom='',requested_cves=None,scanner_candidates=None):
 ...
 return {'binary_metadata':inventory(context) if context else None,'firmware_inventory':reports(context) if context else [],'symptom':symptom,'components':components,'candidates':candidates,'scanner_candidates':scanner_notes,'intake_questions':intake_questions,'scope':'Three reviewed engineering profiles; other requested CVEs enter general investigation. Automatic component discovery is still limited to this catalog; no match does not mean no vulnerabilities.','symptom_causation':'NOT_ESTABLISHED'}
