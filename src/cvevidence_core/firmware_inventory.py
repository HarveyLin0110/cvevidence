...
def _cat(image, member, timeout):
    ...
    with tempfile.TemporaryDirectory() as directory:
        with tempfile.TemporaryFile() as output, tempfile.TemporaryFile() as errors:
            ...
            output.seek(0); data = output.read(MAX_TEXT + 1)
    ...
    for source in context.sources.values():
        ...
        try:
            ...
            if (not isinstance(receipt,dict) or receipt.get('schema_version')!='1.0'
                    ...
                    or not isinstance(receipt.get('files'),list) or len(receipt['files'])>len(PATHS)
            ...
            for row in receipt['files']:
                if (not isinstance(row,dict) or row.get('image_path') not in PATHS
                        or row.get('status') not in ('READ','NOT_READ','TIME_LIMIT','SIZE_LIMIT','UNSUPPORTED_TEXT','ISOLATION_UNAVAILABLE')):
                ...
                if row['status'] == 'READ':
                    if type(row.get('size')) is not int or not 0<=row['size']<=MAX_TEXT:
...
def _child():
    import resource
    if len(sys.argv) != 3 or sys.argv[2] not in PATHS:
        raise SystemExit(2)
    tool = shutil.which('unsquashfs', path=os.defpath)
    isolation = shutil.which('bwrap', path=os.defpath)
    ...
    os.execve(isolation,[isolation,'--unshare-all','--die-with-parent','--new-session','--cap-drop','ALL',
                    ...
                    '/reader','-cat','-no-wildcards','-no-progress','-processors','1',
                    '-data-queue','4','-frag-queue','4','/firmware',sys.argv[2]],
