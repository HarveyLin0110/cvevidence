...
STATES = {'PLANNED': '待開始', 'MATERIALS_AVAILABLE': '可先查閱已提交材料',
          'WAITING_EVIDENCE': '尚未定位，先調查而非立即補件'}
...
def summary_lines(entry):
    ...
    for q in (entry.get('query_plan') or {}).get('queries', []):
        output += [q['query_id'] + ' · ' + q['pc_layer'] + ' · ' + STATES.get(q['status'], q['status']),
                   q['description'], '可用材料種類（不是補件要求）：' + q['materials'], q['reason'],
...
def render(st, entry):
    ...
    st.info('工具尚無此 CVE 的正式驗證規則。以下只是材料索引，不是要你全部補齊；AI 會先讀現有資料，再區分未查完、工具限制與真正缺件。')
    st.subheader('本次查核計畫')
    for q in entry['query_plan']['queries']:
        with st.expander(q['query_id'] + ' · ' + q['pc_layer'] + ' · ' + STATES.get(q['status'], q['status']), expanded=False):
            st.text(q['description'])
            st.text('為何需要：' + q['reason'])
            st.text('可用材料種類（不是補件要求）：' + q['materials'])
            if q['available_source_count']:
                ...
                st.text('尚未依檔案分類定位；應先由工具查找其他路徑，不能僅憑分類認定缺件。')
