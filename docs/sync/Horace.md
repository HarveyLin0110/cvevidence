...
## 2026-09-26 PR #32 兩項 P0 收尾

- 82de159 修正一般 CI 隔離不可用時的預期，專用環境仍必須實際解析；push 36217324358 與 PR 36217326804 全部成功。
- 完成原生 filechooser／HTTP 多檔 ROM＋SBOM＋SDK、21 MiB 大型 ROM、補件、重新分析、祖先 AI 接續及報告下載；上傳 hash、全部原材料與舊 run／AI 均核對保留。
- 實測發現接續已核對的舊 READ 未被多副本門檻計入，首次 32ece4b7 因重讀達 token 門檻。修正只沿用可重新核對的成功 READ，SEARCH／失敗／變更原文不抵用；相同材料及祖先重驗 295a9fcd，4 calls、56.201 秒、141542 tokens，全部步驟成功且未提高預算。
- 全量 676 passed、23 subtests passed（752.30 秒）。完整紀錄見 docs/releases/2026-09-26-P0-CI與原生上傳閉環.md。最終提交 CI 待推送後確認。只更新原 PR，由 Harvey 決定合併；本輪範圍之外仍暫停。

## 2026-09-26 本輪收尾與暫停

- 依使用者要求，不再探索新問題。本輪已知收尾：2cb4a05 閱讀容量、e2e4beb 封存檔內 ROM 盤點、9607aff 統一材料入口與同案件補件指引。
- 最終全量 669 passed、23 subtests passed（265.32 秒）。真實新製 SquashFS 混合 ZIP 能解析清單；直接／封存檔 ROM 共用三份上限，不遞迴解內層封存檔。
- 實連 8506 切換一般多檔／大型單檔及說明正常。未重驗原生選檔 HTTP 上傳，本輪無新 LIVE API。詳見 docs/releases/2026-09-26-混合產品材料收件收尾.md。
- 只推 codex/horace-development，Harvey 決定合併。交付後暫停主動開發，等使用者下次接續；公司內網、可信建置及更廣格式／案例品質仍未完成，不宣稱整體目標完成。

## 2026-09-26 產品片段容量限制

- 2cb4a05：AI READ／初始材料每段 24000 UTF-8 bytes；SEARCH 回傳原文合計 96000，超大上下文退回命中單行，過大命中保留位置並繼續其他來源。COMPARE 也有回傳上限；舊引用核對不變。
- 六項新增案例；完整 665 passed、23 subtests passed（265.17 秒）。首次全量一筆舊 mock 入口失敗，修正後通過。詳見 docs/releases/2026-09-26-產品片段容量限制.md。
- 8506 首頁實連成功，側欄操作未可靠完成；本輪無新 LIVE／原生上傳驗收。只推個人分支。使用者要求收斂：只完成已知混合材料入口及 ZIP 內 ROM 盤點，之後暫停新增調查。

## 2026-09-26 編譯資料庫與補件進展

- a7f44d5：有界解析 compile_commands.json，保留 directory／file／output 宣告、路徑候選與歧義；不執行命令或認證編譯／連結。候選盤點、AI 與網頁資料確認面板共用結果，需引用仍 READ 原文。
- 十六項新增案例，聚焦 18 passed；全量 659 passed、23 subtests passed（270.30 秒）。上輪 CI 36214143515／36214218233 成功，本輪待執行。
- 補件後實際接續 f5b0e484 → fdf8347b：5 calls、67.34 秒、161021 tokens，全步驟成功。把候選縮為「宣告選用 legacy」，明說未執行、未證明成品採用；不重索資料庫，下一步僅一份 final linker map。parent bytes／原 AI 不變。
- 已實連 8506 核對宣告面板與 AI 新進展，詳見 docs/releases/2026-09-26-編譯資料庫與補件進展.md。公司真實建置關聯、來源認證與內網部署仍未完成；只推個人分支。

## 2026-09-26 多副本搜尋與補件門檻

- 41af4fa：搜尋優先回傳不同檔案，保留範圍／跳過／未查數；有界掃描 500 候選／16 MB。89b9973：ASK_USER 前要求已命中同名程式碼副本各有成功 READ，不把 SEARCH 定位當本文閱讀；不是完整語意或副本覆蓋保證。
- 九項新增測試；最終完整 643 passed、23 subtests passed（293.67 秒），聚焦 24 passed。上輪 CI 36213302696／36213329689 成功，本輪待執行；fetch main 無未納入更新。
- 新製 current／legacy MiniZip 双副本：首次只讀 current 就補件，加入檢查後 LIVE f5b0e484 讀完兩份關鍵段落，分辨有／無 0xffff 防護，7 calls、112.03 秒、161349 tokens。一次錯誤 X-ID 被拒絕後修正；只要求一份 compile_commands 作選擇線索，不等於成品綁定認證。
- 實連 8506 核對副本差異與單項補件。完整限制與首次不足見 docs/releases/2026-09-26-多副本搜尋與補件門檻.md；公司真實建置驗證、更多 ROM、跨案例穩定性與內網部署仍未完成。
